...
os.chdir("/home/liujun/BHM/mvts_transformer")  # 把目录切换到当前项目，这句话是关键
import pandas as pd
...
def generate_noised_data(args: argparse.Namespace):
    ...
    clabel_train = np.asarray(label_train, dtype=np.float32)
    clabel_eval = np.asarray(label_eval, dtype=np.float32)
    clabel_test = np.asarray(label_test, dtype=np.float32)
